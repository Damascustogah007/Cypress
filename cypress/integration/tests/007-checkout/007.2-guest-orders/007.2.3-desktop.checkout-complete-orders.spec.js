...
describe(`${testSpecName} - Desktop (${namedViewports.desktop.join(
  'x',
  ...
  it('should add one item to basket and show one item in basket page', () => {
    ...
    cy.get('[data-cy*="basket-product_val-"]').should('have.length', 1)
  })
  ...
  it('Can go back to homepage and is not logged in', () => {
    ...
    cy.dataCy('welcome-salutation').should(
      'not.contain',
      registration_guest_no_password.lastName,
    )
  })
...
describe(`${testSpecName} - Desktop (${namedViewports.desktop.join(
  ...
  it('Cannot register with existing mail', () => {
    cy.intercept({
      method: 'POST',
      url: 'https://**/Customer/CustomerSave',
    }).as('customerSaveGuest')
    ...
    cy.wait('@customerSaveGuest').then((data) => {
      expect(data.response.statusCode, 'Response Status Code').to.equal(200)
      expect(data.response.body.ResultStatus.Error, 'Response Result Status Error').to.be.true
    })
  })
...
describe(`${testSpecName} - Desktop (${namedViewports.desktop.join(
  ...
  it('Can go back to homepage and is not logged in', () => {
    ...
    cy.location('pathname').should('equal', '/de/')
    cy.dataCy('welcome-salutation').should('not.contain', newCustomer.lastName)
  })
  ...
  it('Can go to order confirmation', () => {
    ...
    cy.dataCy('checkout-payment-button-submit').first().click() -
      cy
        .get('[data-cy="checkout-confirmation-address"]')
        .should('exist')
